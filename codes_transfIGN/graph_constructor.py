...
from dgllife.utils import BaseAtomFeaturizer, atom_type_one_hot, atom_degree_one_hot, atom_total_num_H_one_hot, \
    atom_is_aromatic, ConcatFeaturizer, bond_type_one_hot, atom_hybridization_one_hot, atom_formal_charge_one_hot, atom_explicit_valence_one_hot, atom_implicit_valence_one_hot,\
...
class P_AtomFeaturizer(BaseAtomFeaturizer):
    def __init__(self, atom_data_field='h'):
        SYMBOLS = ['C', 'N', 'O', 'P', 'S', 'Fe', 'Zn', 'Cu', 'B']

        super(P_AtomFeaturizer, self).__init__(
            featurizer_funcs={
...
AtomFeaturizer = P_AtomFeaturizer()
...
def graphs_from_mol(m1, m2, add_self_loop=False, add_3D=False):
    ...
    # efeats1
    efeats1 = BondFeaturizer(mol1)['e']  
    g1.edata['e'] = torch.cat([efeats1[::2], efeats1[::2]])

    # efeats2
    efeats2 = BondFeaturizer(mol2)['e'] 
